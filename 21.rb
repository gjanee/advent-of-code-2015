...
class Item
  ...
  def initialize(cost, damage:nil, armor:nil)
...
Weapons = [
  Item.new(8,   damage:4),
  Item.new(10,  damage:5),
  Item.new(25,  damage:6),
  Item.new(40,  damage:7),
  Item.new(74,  damage:8)]

Armors = [
  Item.new(0,   armor:0), # no armor option
  Item.new(13,  armor:1),
  Item.new(31,  armor:2),
  Item.new(53,  armor:3),
  Item.new(75,  armor:4),
  Item.new(102, armor:5)]

Rings = [
  Item.new(25,  damage:1),
  Item.new(50,  damage:2),
  Item.new(100, damage:3),
  Item.new(20,  armor:1),
  Item.new(40,  armor:2),
  Item.new(80,  armor:3)]
...
class Player
  ...
  attr_reader :points, :damage, :armor

  def initialize(points, damage, armor)
    @points, @damage, @armor = points, damage, armor
  end

end

boss = Player.new(*open("21.in").read.scan(/\d+/).map(&:to_i))

class Outcome

  attr_reader :cost

  def initialize(cost, i_win)
    @cost, @i_win = cost, i_win
  end
...
Weapons.each do |w|
  Armors.each do |a|
    (0..2).each do |nr|
      Rings.combination(nr).each do |rings|
        cost = w.cost + a.cost + rings.map {|r| r.cost }.reduce(0, :+)
        damage = w.damage +
          rings.select {|r| r.offensive? }.map {|r| r.damage }.reduce(0, :+)
        armor = a.armor +
          rings.select {|r| r.defensive? }.map {|r| r.armor }.reduce(0, :+)
        me = Player.new(100, damage, armor)
        my_attack = [me.damage-boss.armor, 1].max
        boss_attack = [boss.damage-me.armor, 1].max
        outcomes << Outcome.new(cost,
          boss.points/my_attack <= me.points/boss_attack)
      end
    end
  end
end

puts outcomes.select {|o| o.i_win? }.min.cost
...
puts outcomes.select {|o| !o.i_win? }.max.cost
