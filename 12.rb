...
def hash_sum(node)
  sum(node.keys)+sum(node.values)
end

def sum(node)
  return node if node.kind_of?(Numeric)
  return 0 if node.kind_of?(String)
  return node.map {|n| sum(n) }.reduce(0, :+) if node.kind_of?(Array)
  return hash_sum(node) if node.kind_of?(Hash)
end

document = JSON.parse(open("12.in").read)
puts sum(document)
...
def hash_sum(node)
  node.values.member?("red") ? 0 : sum(node.keys)+sum(node.values)
end
