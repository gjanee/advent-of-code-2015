...
# her a thank you card.  However, there's a small problem: she signed
# it "From, Aunt Sue".
#
...
def match(sue)
  sue.map {|c,v| v == MFCSAM[c] }.all?
end

def solve
  open("16.in").each do |l|
    m = /Sue (\d+): (.*)/.match(l)
    id = m[1]
    sue = Hash[
      m[2].split(",")
...
#
# In particular, the cats and trees readings indicate that there are
...
def match(sue)
  sue.map {|c,v|
    if MFCSAM[c].kind_of?(Array)
      MFCSAM[c][0] == ">" ? v > MFCSAM[c][1] : v < MFCSAM[c][1]
    else
      v == MFCSAM[c]
    end
  }.all?
