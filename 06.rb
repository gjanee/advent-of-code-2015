...
def process(&block)
  open("06.in").each_line do |l|
    fx, fy, tx, ty = l.scan(/\d+/).map(&:to_i)
    (fx..tx).each do |x|
      (fy..ty).each do |y|
        if l.start_with?("turn on")
          yield("on", x, y)
        elsif l.start_with?("turn off")
          yield("off", x, y)
        elsif l.start_with?("toggle")
          yield("toggle", x, y)
        end
      end
    end
...
process {|op,x,y|
  ...
  when "on"
    grid[x*1000+y] += 1
  when "off"
    grid[x*1000+y] = [grid[x*1000+y]-1, 0].max
  when "toggle"
    grid[x*1000+y] += 2
  end
}
