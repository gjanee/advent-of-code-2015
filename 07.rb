...
open("07.in").each do |l|
  tokens = l.split.map {|t| t =~ /^\d+$/ ? t.to_i : t }
  $wires[tokens[-1]] = tokens[0..-3]
end

$cache = {}
def evaluate(wire)
  return wire if wire.kind_of?(Integer)
  return $cache[wire] if $cache.member?(wire)
  expr = $wires[wire]
  if expr.length == 1
    v = evaluate(expr[0])
  elsif expr[0] == "NOT"
    v = ~evaluate(expr[1]) & 0xFFFF
  else
    case expr[1]
    when "AND"
      v = evaluate(expr[0]) & evaluate(expr[2])
    when "OR"
      v = evaluate(expr[0]) | evaluate(expr[2])
    when "LSHIFT"
      v = (evaluate(expr[0]) << evaluate(expr[2])) & 0xFFFF
    when "RSHIFT"
      v = evaluate(expr[0]) >> evaluate(expr[2])
    end
  end
